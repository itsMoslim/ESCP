...
function findLatestCoachDocument(string $username, string $prefix): string
{
  $safeUsername = preg_replace('/[^a-zA-Z0-9_-]/', '_', $username);
  $dir = "assets/documents/";
  $pattern = $dir . $prefix . "_" . $safeUsername . "_*.*";
  $files = glob($pattern);
...
if (empty($username)) {
  ...
  if (!$coach) {
    ...
    $idDocument = findLatestCoachDocument($coach['username'], 'id');
    $rankProofDocument = findLatestCoachDocument($coach['username'], 'rank_proof');
                    ...
                    <div class="left-info">
                      ...
                      <?php else: ?>
                        ...
                        <p>
                          ...
                          <?php if (!empty($idDocument)): ?>
                            <a href="assets/documents/<?php echo rawurlencode($idDocument); ?>" target="_blank" rel="noopener noreferrer">View ID</a>
                          <?php else: ?>
                            Empty
                          <?php endif; ?>
                        </p>
                        <p>
                          <strong>Rank Proof:</strong>
                          <?php if (!empty($rankProofDocument)): ?>
                            <a href="assets/documents/<?php echo rawurlencode($rankProofDocument); ?>" target="_blank" rel="noopener noreferrer">View Rank Proof</a>
                          <?php else: ?>
                            Empty
                          <?php endif; ?>
                        </p>
